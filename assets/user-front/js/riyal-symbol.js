/**
 * استبدال كلمة "ريال" برمز الريال (SVG) في الواجهة — تشغيل مرة واحدة بعد تحميل الصفحة بدون ضغط على الأداء
 */
(function () {
  var RIAL_WORD = 'ريال';
  ...
  function run() {
    if (!document.body || document.body.getAttribute('data-riyal-symbol-done')) return;
    document.body.setAttribute('data-riyal-symbol-done', '1');

    var style = document.createElement('style');
    style.textContent = '.riyal-symbol{display:inline-block;vertical-align:middle;width:1em;height:1em;margin:0 .05em;line-height:1}.riyal-symbol svg{width:100%;height:100%;display:block}';
    ...
    while ((node = walker.nextNode())) {
      if (node.textContent && node.textContent.indexOf(RIAL_WORD) !== -1) {
        ...
        if (p && !p.closest('script') && !p.closest('style') && !p.closest('noscript')) toReplace.push(node);
      }
    }
